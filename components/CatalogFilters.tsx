...
const selectClass =
  "w-full appearance-none rounded-lg border border-gray-200 bg-white px-4 py-3.5 text-base text-gray-800 focus:border-purple-600 focus:outline-none";
  ...
  return (
    <div className="border-b border-gray-100">
      ...
      >
        ...
        <svg
          className={`h-4 w-4 text-purple-700 transition-transform ${open ? "rotate-90" : ""}`}
          fill="none"
          stroke="currentColor"
          viewBox="0 0 24 24"
        >
      ...
      <section className="space-y-2">
        <div className="flex items-center gap-3 rounded-full border border-gray-200 bg-white px-4 py-3 sm:px-5 sm:py-3.5">
          ...
          <button
            ...
            className="min-w-0 flex-1 truncate text-left text-base font-medium text-purple-900 hover:text-purple-700 sm:text-lg"
          >
          ...
          <button
            type="button"
            onClick={() => setOpen(true)}
            className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-purple-700 text-white hover:bg-purple-800"
        ...
        <p className="text-sm text-gray-500">
          ...
          {hasActiveFilters(filters) && <span className="ml-2 text-purple-700">· Filtros ativos</span>}
        </p>

        {locationEnabled && (
          <p className="text-sm font-medium text-purple-700">Distâncias calculadas a partir da sua localização.</p>
        )}
          ...
          >
            <div className="flex-1 overflow-y-auto px-5 pt-5">
              ...
              <FilterAccordion
                icon={<span>🏳️</span>}
                title="Nacionalidade"
                options={FILTER_NATIONALITIES}
                selected={draft.selectedNationalities}
                onChange={(v) => updateDraft({ selectedNationalities: v })}
              />
              <FilterAccordion
                icon={<span>👙</span>}
                title="Seios"
                options={FILTER_BREAST_TYPES}
                selected={draft.selectedBreastTypes}
                onChange={(v) => updateDraft({ selectedBreastTypes: v })}
              />
              <FilterAccordion
                icon={<span>💇</span>}
                title="Cabelo"
                options={FILTER_HAIR_TYPES}
                selected={draft.selectedHairTypes}
                onChange={(v) => updateDraft({ selectedHairTypes: v })}
              />
              <FilterAccordion
                icon={<span>🧍</span>}
                title="Tipo de corpo"
                options={FILTER_BODY_TYPES}
                selected={draft.selectedBodyTypes}
                onChange={(v) => updateDraft({ selectedBodyTypes: v })}
              />
              <FilterAccordion
                icon={
                  <svg className="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <div className="space-y-4 border-t border-gray-100 py-4">
                <label className="flex items-center gap-3 text-base font-medium text-gray-800">
                  <input
                    ...
                    className="h-5 w-5 rounded text-purple-700"
                  />
                  Só verificadas
                </label>
            ...
            <div className="flex items-center justify-between border-t border-gray-200 px-5 py-4">
              <button
                ...
                className="text-sm font-black uppercase tracking-wide text-purple-700 hover:text-purple-900"
              >
                Excluir tudo
              </button>
              ...
              <button
                ...
                className="flex items-center gap-2 rounded-full bg-purple-700 px-6 py-3 text-sm font-black uppercase tracking-wide text-white hover:bg-purple-800"
              >
